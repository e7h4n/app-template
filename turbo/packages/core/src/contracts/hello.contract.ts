...
export const HelloRequestSchema = z.object({
  ...
  language: z.enum(['en', 'zh', 'ja', 'es']).optional().default('en').describe('Preferred language for greeting'),
  includeTime: z.boolean().optional().default(false).describe('Include current time in response'),
});
...
export const helloContract = c.router({
  sayHello: {
    ...
    description: 'Creates a personalized greeting message based on the provided name and language preference',
  },
  ...
  getGreeting: {
    ...
    query: z.object({
      lang: z.enum(['en', 'zh', 'ja', 'es']).optional().describe('Language for greeting'),
    }),
    responses: {
      200: z.object({
        greeting: z.string(),
